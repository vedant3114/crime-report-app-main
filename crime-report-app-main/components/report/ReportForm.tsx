...
export function ReportForm({ onCompleteAction }: ReportFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate image
    if (!image) {
      toast.error("Please upload a photo for your report");
      setIsSubmitting(false);
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="relative group">
        ...
        <label
          ...
          className={`block w-full p-8 border-2 border-dashed rounded-2xl 
                   hover:border-sky-500/50 hover:bg-sky-500/5 transition-all duration-200
                   cursor-pointer text-center ${
                     image ? 'border-sky-500/50' : 'border-zinc-700'
                   }`}
        >
          {image ? (
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-zinc-400">
                Drop an image here or click to upload <span className="text-red-500">*</span>
              </p>
            </div>
          )}
        </label>
        ...
        {!image && (
          <p className="text-sm text-red-500 mt-2">Please upload a photo for your report</p>
        )}
      </div>

      {/* Emergency Detection Warning */}
      {isEmergency === false && (
        <div className="flex items-center gap-2 text-yellow-400 text-sm bg-yellow-500/10 p-4 rounded-xl border border-yellow-500/20">
          <svg
          ...
          <span>
            The image analysis indicates this is not an emergency situation. Please ensure this is the correct image for your report.
          </span>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        />
        {!formData.location && (
          <p className="text-sm text-red-500">Please select a location for your report</p>
        )}
      </div>
      ...
      <button
        type="submit"
        disabled={isSubmitting || (isEmergency === false && formData.incidentType === "EMERGENCY")}
        className={`w-full relative group overflow-hidden rounded-xl bg-gradient-to-br from-sky-500 to-blue-600 
                 ...
                 ${isEmergency === false && formData.incidentType === "EMERGENCY" ? "bg-zinc-700 hover:bg-zinc-600" : ""}`}
      >
        <div className="relative flex items-center justify-center gap-2">
          {isSubmitting ? (
            ...
            <>
              <span>
                {isEmergency === false && formData.incidentType === "EMERGENCY"
                  ? "Cannot Submit Emergency Report"
                  : "Submit Report"}
              </span>
              <svg
